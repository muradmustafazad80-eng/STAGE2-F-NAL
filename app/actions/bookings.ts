...
export async function createBooking(input: CreateBookingInput): Promise<BookingResult> {
  ...
  try {
    ...
    const [service] = await db.select({ id: services.id, price: services.price, duration: services.duration }).from(services).where(and(eq(services.id, parsed.data.serviceId), eq(services.businessId, business.id), eq(services.status, 'active'))).limit(1)
